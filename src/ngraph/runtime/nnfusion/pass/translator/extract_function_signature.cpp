...
using namespace ngraph::runtime::nnfusion::translator;
...
bool ExtractFunctionSignature::extract_constants(std::shared_ptr<FunctionTranslatorContext> ctx,
...
void ExtractFunctionSignature::propagate_in_place_input(
...
void ExtractFunctionSignature::propagate_in_place_output(
    std::shared_ptr<FunctionTranslatorContext> ctx,
...
bool ExtractFunctionSignature::extract_args(std::shared_ptr<FunctionTranslatorContext> ctx,
                                            std::shared_ptr<TranslationUnit> tu,
                                            std::shared_ptr<ngraph::Function> function)
{
    size_t arg_index = 0;
    for (shared_ptr<ngraph::op::Parameter> param : function->get_parameters())
    {
        for (size_t i = 0; i < param->get_output_size(); ++i)
        {
            ...
            NGRAPH_DEBUG << "Param Tensor:\t" << tv->get_name() << "\twith id: " << ss.str()
                         << endl;
        }
    }
    return true;
...
bool ExtractFunctionSignature::extract_output(std::shared_ptr<FunctionTranslatorContext> ctx,
                                              std::shared_ptr<TranslationUnit> tu,
                                              std::shared_ptr<ngraph::Function> function)
{
    for (size_t i = 0; i < function->get_output_size(); ++i)
    {
        ...
        if (!input_node->is_constant() && !input_node->is_parameter())
        {
            ...
            NGRAPH_DEBUG << "Output Tensor:\t" << itv->get_name() << "\t with id:" << output_name
                         << endl;
        }
    }
    return true;
...
bool ExtractFunctionSignature::run(std::shared_ptr<FunctionTranslatorContext> ctx,
                                   std::shared_ptr<TranslationUnit> tu,
                                   std::shared_ptr<ngraph::Function> function)
{
