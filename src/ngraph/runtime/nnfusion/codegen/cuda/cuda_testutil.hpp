            ...
            namespace codegen
            {
                ...
                void test_cudaMalloc(CodeWriter& writer, const TensorWrapper tensor)
                {
                    writer << tensor.get_type() << "* " << tensor.get_name() << ";\n"
                           << "cudaMalloc((void**)&"<< tensor.get_name() << "," << tensor.get_size()*tensor.get_type().size() << ");\n";
                }
                ...
                vector<float> test_hostData(CodeWriter& writer, const TensorWrapper tensor, vector<float>& d)
                {
                    size_t size = tensor.get_size();
                    vector<string> dstr;
                    for(int i=0; i<size; i++)
                ...
                void test_compare(CodeWriter& writer, const TensorWrapper tensor)
                {
                    ...
                    writer << "for(int i = 0; i < " << size <<"; i++)\n"
                           << "{\n"
                           << "    if(abs("<<tensor.get_name()<<"_host_result[i] - "<< tensor.get_name()<<"_host[i]) > 0.00005)\n"
                           << "    {\n"
