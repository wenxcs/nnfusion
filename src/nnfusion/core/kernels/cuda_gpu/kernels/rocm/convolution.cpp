...
cuda::RocmConvolutionCudnn::RocmConvolutionCudnn(shared_ptr<KernelContext> ctx)
    : CudaLibEmitter(ctx)
{
...
LanguageUnit_p cuda::RocmConvolutionCudnn::emit_function_body()
{
...

    {
        ...
        lu << "static bool inited = false; static int returnedAlgoCount, fastest = 0;\n";
        lu << "static miopenConvAlgoPerf_t perfResults[4];";
        lu << "static size_t workspace_size_in_bytes = 0;\n";
        lu << "static void *workspace_ptr = NULL;\n";
        lu << "const float alpha = 1.0;\n";
        lu << "const float beta = 0.0;\n";
        lu << "if (!inited) { inited = true; fprintf(stderr, \"[MIOpen] Convolution running "
              "auto-tune for Forward-Data;\\n\"); "
              "CUDNN_SAFE_CALL(miopenFindConvolutionForwardAlgorithm(global_cudnn_handle, "
              "tensor_desc_0, input0, filter_desc, input1, conv_desc, tensor_desc_1, output0, 4, "
              "&returnedAlgoCount, perfResults, workspace_ptr, workspace_size_in_bytes, false)); "
              "for (int i = 1; i < returnedAlgoCount; ++i) if (perfResults[i].time < "
              "perfResults[fastest].time) fastest = i; workspace_size_in_bytes = "
              "perfResults[fastest].memory; CUDA_SAFE_CALL(cudaMalloc(&workspace_ptr, "
              "workspace_size_in_bytes)); }\n";

        lu << "CUDNN_SAFE_CALL(miopenConvolutionForward(global_cudnn_handle, "
           << "&alpha, "
           << "tensor_desc_0, "
           << "input0,"
           << "filter_desc, "
...
LanguageUnit_p cuda::RocmConvolutionCudnn::emit_dependency()
{
