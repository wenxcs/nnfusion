    ...
    namespace kernels
    {
        namespace cuda
        {
            ...
            public:
                ...
                {
                    input_size = ctx->get_input_tensor(0).size(false);
                    output_size = ctx->get_output_tensor(0).size(false);
                    input_outer_dim_size = (ctx->get_input_tensor(1).get_shape())[0];
                    input_inner_dim_size =
                        ctx->get_input_tensor(0).size(false) / input_outer_dim_size;

                    input0_t = ctx->get_input_tensor(0).get_element_type().c_type_string();
                    input1_t = ctx->get_input_tensor(1).get_element_type().c_type_string();
                    output0_t = ctx->get_output_tensor(0).get_element_type().c_type_string();
