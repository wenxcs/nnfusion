        ...
        namespace cuda
        {
            ...
            public:
                    ...
                    : KernelEmitter(ctx, "cuda_sp")
                {
                    ...
                    {
                        auto arg_rank = ctx->get_input_tensor(i).get_shape().size();
                        for (size_t j = concat_axis; j < arg_rank; j++)
                        {
                            inputs_strides[i] *= ctx->get_input_tensor(i).get_shape()[j];
                        }
                    ...
                    for (uint32_t i = 0; i < input_num; i += split_input_size)
                    {
                        ...
                        {
                            nthread += shape_size(ctx->get_input_tensor(j).get_shape());
                    ...
                    tag << "_s" << join(ctx->get_output_tensor(0).get_shape(), "_") << "_a_"
                        << concat_axis;
                    for (size_t i = 0; i < input_num; i++)
                    {
                        tag << "_i_" << join(ctx->get_input_tensor(i).get_shape(), "_");
                    }
                    custom_tag = tag.str();
                }
