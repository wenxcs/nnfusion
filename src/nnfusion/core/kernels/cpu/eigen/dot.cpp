...
LanguageUnit_p cpu::Dot::emit_function_body()
{
    ...
    lu << m_context->get_input_tensor(0).get_element_type().c_type_string() << "* "
       << m_context->get_input_tensor(0).get_name() << " = input0;\n";
    lu << m_context->get_input_tensor(1).get_element_type().c_type_string() << "* "
       << m_context->get_input_tensor(1).get_name() << " = input1;\n";
    lu << m_context->get_output_tensor(0).get_element_type().c_type_string() << "* "
       << m_context->get_output_tensor(0).get_name() << " = output0;\n";

    if (arg0_shape.empty() || arg1_shape.empty())
    {
        auto& first =
            (arg0_shape.empty() ? m_context->get_input_tensor(0) : m_context->get_input_tensor(1));
        auto& second =
            (arg0_shape.empty() ? m_context->get_input_tensor(1) : m_context->get_input_tensor(0));

        lu.block_begin();
        lu << emit_eigen_vector(m_context->get_output_tensor(0)) << "\n    = ";
    ...
    else if ((arg0_shape.size() == 1) && (arg1_shape.size() == 1) && reduction_axes == 1)
    {
        lu.block_begin();
        lu << emit_eigen_vector(m_context->get_output_tensor(0)) << " = \n"
           << "    " << emit_eigen_vector(m_context->get_input_tensor(0)) << ".dot("
           << emit_eigen_vector(m_context->get_input_tensor(1)) << ");\n";
        lu.block_end();
    }
    else if ((arg0_shape.size() == 2) && (arg1_shape.size() == 1) && reduction_axes == 1)
    {
        lu.block_begin();
        lu << emit_eigen_vector(m_context->get_output_tensor(0)) << " = \n"
           << "    " << emit_eigen_matrix(m_context->get_input_tensor(0)) << " * "
           << emit_eigen_vector(m_context->get_input_tensor(1)) << ";\n";
        lu.block_end();
    }
    ...
    {
        lu.block_begin();
        lu << emit_eigen_matrix(m_context->get_output_tensor(0)) << " = \n"
           << "    " << emit_eigen_matrix(m_context->get_input_tensor(0)) << " * "
           << emit_eigen_matrix(m_context->get_input_tensor(1)) << ";\n";
        lu.block_end();
    }
    else
    {
