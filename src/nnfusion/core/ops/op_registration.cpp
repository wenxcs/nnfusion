...
namespace ngraph
{
    namespace op
    {
		const std::unordered_map<std::string, OpConfig> &ensure_initialize_op_configs() {
			...
			op_configs["BatchMatmul"] = OpConfig()
				...
				.infershape([](GenericOp &target_op) -> void {
					...
					int m0 = input_shape_0[input_shape_0.size() - 2], n0 = input_shape_0[input_shape_0.size() - 1];
					int m1 = input_shape_1[input_shape_1.size() - 2], n1 = input_shape_1[input_shape_1.size() - 1];
