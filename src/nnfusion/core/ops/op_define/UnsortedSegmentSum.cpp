...
REGISTER_OP(UnsortedSegmentSum).infershape([](ngraph::op::GenericOp& target_op) -> void {
    GENERIC_OP_LOGGING();
    CHECK(target_op.get_input_size() == 3) << "Inputs of UnsortedSegmentSum should be 3.";
    ...
    CHECK(ng_seg->description() == "Constant") << "We only accept the sgements number as Constant.";
