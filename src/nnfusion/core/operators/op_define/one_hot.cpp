...
OneHot::OneHot(const ngraph::PartialShape& shape, size_t one_hot_axis)
    : Op("OneHot")
    , m_shape(shape)
...
{
    ngraph::element::Type arg_et = gnode->get_input_element_type(0);
    ngraph::PartialShape arg_shape = gnode->get_input_partial_shape(0);
    ngraph::Rank arg_rank = arg_shape.rank();
    ...
    ngraph::PartialShape result_shape{m_shape};

    if (arg_rank.is_static())
    {
        std::vector<ngraph::Dimension> expected_input_dims(static_cast<size_t>(m_shape.rank()));
        ...
        ngraph::PartialShape expected_input_shape{expected_input_dims};

        ngraph::PartialShape merged_input_shape{expected_input_shape};
        OP_VALIDATION(this, ngraph::PartialShape::merge_into(merged_input_shape, arg_shape))
            << "Argument shape " << arg_shape << " does not match the expected shape of "
            << expected_input_shape << ".";

        std::vector<ngraph::Dimension> output_dims(static_cast<size_t>(merged_input_shape.rank()));
