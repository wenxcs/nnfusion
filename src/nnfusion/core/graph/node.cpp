    ...
    , m_unique_name(description() + "_" + to_string(m_instance_id))
{
    ...
    {
        for (ngraph::descriptor::Output& output : arg->get_outputs())
        {
            // TODO
...
std::deque<ngraph::descriptor::Output>& Node::get_outputs()
{
    return m_outputs;
}

const std::deque<ngraph::descriptor::Output>& Node::get_outputs() const
{
    return m_outputs;
}
...
ngraph::Placement Node::get_placement() const
{
    return m_placement;
}

void Node::set_placement(ngraph::Placement placement)
{
...
/*
TODO:
Output.get_node will return ngraph::Node instead of nnfusion::Node
...
std::vector<std::shared_ptr<ngraph::Function>> Node::get_functions() const
{
    return std::vector<std::shared_ptr<ngraph::Function>>{};
}
...
static std::string pretty_element_type(const ngraph::element::Type& et)
{
    if (et.is_dynamic())
    {
        return "?";
...
const ngraph::element::Type& Node::get_output_element_type(size_t i) const
{
    return m_outputs.at(i).get_element_type();
}

const ngraph::element::Type& Node::get_element_type() const
{
...
const ngraph::Shape& Node::get_output_shape(size_t i) const
{
    return m_outputs.at(i).get_shape();
}

const ngraph::PartialShape& Node::get_output_partial_shape(size_t i) const
{
    return m_outputs.at(i).get_partial_shape();
}

const ngraph::Shape& Node::get_shape() const
{
    if (get_output_size() != 1)
    {
        stringstream es;
...
shared_ptr<ngraph::descriptor::Tensor> Node::get_output_tensor_ptr(size_t i) const
{
    return m_outputs.at(i).get_tensor_ptr();
}

shared_ptr<ngraph::descriptor::Tensor> Node::get_output_tensor_ptr() const
{
    if (get_output_size() != 1)
    {
...
const std::set<ngraph::descriptor::Input*>& Node::get_output_inputs(size_t i) const
{
    return m_outputs.at(i).get_inputs();
}

ngraph::descriptor::Tensor& Node::get_output_tensor(size_t i) const
{
    return m_outputs.at(i).get_tensor();
}

ngraph::descriptor::Tensor& Node::get_output_tensor() const
{
    if (get_output_size() != 1)
    {
...
const ngraph::element::Type& Node::get_input_element_type(size_t i) const
{
    return m_inputs.at(i).get_element_type();
}

const ngraph::Shape& Node::get_input_shape(size_t i) const
{
    return m_inputs.at(i).get_shape();
}

const ngraph::PartialShape& Node::get_input_partial_shape(size_t i) const
{
...
ngraph::descriptor::Input* Node::get_input_from(const shared_ptr<Node>& src)
{
    ...
    throw ngraph::ngraph_error("Error: src is not one of self's input Node");
}

ngraph::descriptor::Output* Node::get_output_to(const shared_ptr<Node>& dst)
{
    ...
    throw ngraph::ngraph_error("Error: dst is not one of self's output Node");
}
...
void nnfusion::graph::check_new_args_count(const Node* node, const NodeVector& new_args)
{
...
const NodeVector& nnfusion::graph::check_single_output_args(const NodeVector& args)
{
    for (size_t i = 0; i < args.size(); ++i)
    {
        nnfusion::graph::check_single_output_arg(args.at(i), i);
    }
    return args;
}

std::tuple<ngraph::element::Type, ngraph::PartialShape> Node::validate_and_infer_elementwise_args()
{
...
void Node::validate_and_infer_elementwise_arithmetic()
{
    ...
    NODE_VALIDATION_ASSERT(this, args_et.is_dynamic() || args_et != ngraph::element::boolean)
...
void Node::validate_and_infer_elementwise_logical()
{
    ...
    set_output_type(0, ngraph::element::boolean, args_pshape);
}
...
bool Node::validate_punt_if_dynamic()
{
    ...
    if (any_dynamic)
    {
        for (size_t i = 0; i < get_output_size(); i++)
        {
            set_output_type(i, ngraph::element::dynamic, ngraph::PartialShape::dynamic());
        }
        return true;
    }
    else
