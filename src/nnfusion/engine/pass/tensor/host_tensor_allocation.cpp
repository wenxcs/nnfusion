                               ...
                               std::shared_ptr<TranslationUnit> tu)
{
        ...
        for (auto& ins : *iterator)
        {
            ...
            for (size_t i = 0; i < kernel_context->inputs.size(); i++)
            {
                ...
                if (outwrapper != output_wrappers.end())
                {
                    ...
                    NNFUSION_LOG(INFO) << "Replacing " << gnode->get_name() << " "
                                       << tdesc->get_name() << " with new host tensor "
                                       << newts->second->get_name() << ".";
    ...
    // Liveness again, we put those mirror gnode in the same place.
    for (auto iterator : p)
    {
        for (auto& ins : *iterator)
        {
            auto gnode = ins->getGNode();
            for (auto& ts : gnode->liveness_new_list)
            {
                auto mirror = tensor_mirror_map.find(ts);
                if (mirror != tensor_mirror_map.end())
                    gnode->liveness_new_list.insert(mirror->second.get());
            }

            for (auto& ts : gnode->liveness_free_list)
            {
                auto mirror = tensor_mirror_map.find(ts);
                if (mirror != tensor_mirror_map.end())
                    gnode->liveness_free_list.insert(mirror->second.get());
            }
        }
    }
    return true;
