        ...
        namespace graph
        {
            class RuntimeConstantFoldingPass : public GraphPassBase
            {
                    ...
                    std::set<std::shared_ptr<GNode>>& blocklist_nodes)
                {
                    ...
                    {
                        NNFUSION_LOG(INFO)
                            << ">> Found constant downstream node: " << it->get_name()
                        ...
                        for (auto& input : it->get_in_edges())
                        {
                            ...
                            NNFUSION_LOG(INFO)
                                << "  With Constant Input Node: " << p_const->get_name()
            ...
            public:
                bool run_on_graph(std::shared_ptr<Graph>& graph) override
                {
                    ...
                    do
                    {
                        ...
                        NNFUSION_LOG(INFO)
                            << ">> Runtime One Iteration Folds Infer-able Node Count: "
                            << folding_cnt;
                    } while (folding_cnt > 0);
                    NNFUSION_LOG(INFO) << "";
                    NNFUSION_LOG(INFO) << ">> Runtime Constant Folding Pass ends for Graph: "
                                       << graph->get_name();
                    NNFUSION_LOG(INFO) << "";
                    return true;
                }
