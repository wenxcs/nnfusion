...
TEST(ngraph_op, sum)
{
    ...
    auto A = make_shared<op::Parameter>(element::f32, shape_a);
    auto args = shared_ptr<ngraph::Node>(A);
    ngraph::AxisSet reduce_axes;

    // Create node
    auto node = std::make_shared<ngraph::op::Sum>(args, reduce_axes);
...
TEST(ngraph_op, broadcast)
{
    ...
    // Create node
    auto node = std::make_shared<ngraph::op::Broadcast>(args, output_shape, broadcast_axes);
